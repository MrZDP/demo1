...
void OLED_DisplayTurn(u8 i)
{
	if(i==0)
		{
			OLED_Send_Command(0xC8);//正常显示
			OLED_Send_Command(0xA1);
		}
	if(i==1)
		{
			OLED_Send_Command(0xC0);//反转显示
...
{
	OLED_Send_Command(0x8D);//电荷泵使能
	OLED_Send_Command(0x14);//开启电荷泵
	OLED_Send_Command(0xAF);//点亮屏幕
}
...
{
	OLED_Send_Command(0x8D);//电荷泵使能
	OLED_Send_Command(0x10);//关闭电荷泵
	OLED_Send_Command(0xAE);//关闭屏幕
}
...
void OLED_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
	   OLED_Send_Command(0xb0+i); //设置行起始地址
	   OLED_Send_Command(0x00);   //设置低列起始地址
	   OLED_Send_Command(0x10);   //设置高列起始地址
	   for(n=0;n<128;n++)
